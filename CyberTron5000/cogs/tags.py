...
class Tags(commands.Cog):
    ...
    async def tag(self, ctx, *, name=None):
        ...
        tag = await self.client.pg_con.fetch("SELECT content, uses FROM tags WHERE name = $1 AND guild_id = $2", name,
                                             str(ctx.guild.id))
        all_tags = await self.client.pg_con.fetch("SELECT name FROM tags WHERE guild_id = $1", str(ctx.guild.id))
        if not tag:
    ...
    async def create(self, ctx, *, name):
        """Creates a tag"""
        self.forbidden = tuple([f"{command.name} " for command in self.client.get_command('tag').commands])
        if len(name) > 100:
